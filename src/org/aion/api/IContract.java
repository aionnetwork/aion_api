...
/**
 * A Contract class that sits above the Aion Java API layer that provides the user
 * with convenient methods of encoding and decoding contract calls and
 * transactions from the Aion Kernel. The IContract interface has no public
 * constructors, instead the user utilizes the {@link IContractController IContractController} to create the Contract.
 *
 * <pre>
 * {@code
...
public interface IContract {
    ...
    /**
     * Initiates a new contract function call. Intended usage is to use this
     * function in conjunction with
     * {@link IContract#setParam(ISolidityArg) setParam}, {@link #build() build}
     * and {@link IContract#execute() execute} to execute a contract function call as a
     * chain call. Intended usage is shown as below :
     * <pre>
     * {@code
     * IAionAPI api = IAionAPI.init();
     * IContractController cc = new ContractController(api);
     * cc.createFromSource(contrac_source, deployer, energyLimit, energyPrice)
     ...
     *
     * @param f
     *         name of the function call by {@link java.lang.String String} exactly as it appears in the ABI
     *         definition.
    ...
    /**
     * Sets a parameter of the function, the order in which this function is
     * called must be in order of the ABI definition. Type and size checks are
     * done in {@link #build() build} to ensure parameter order and size are
     * correct.
     *
     * @param val
     *         the Solidity Type like {@link org.aion.api.sol.IInt IInt}, {@link org.aion.api.sol.IBool IBool} or
     *         {@link org.aion.api.sol.ISString ISString}.
     * @return the contract interface {@link IContract IContract}.
     */
     ...
     *
     * @param address
     *         the class {@link Address Address} represent the desired sender account.
     * @return the contract interface {@link IContract IContract}.
     */
    ...
    /**
     * Does checks to ensure the function is correct and assembles the
     * transaction hash. Required for {@link #execute() execute} to operate.
     *
     * @return the contract interface {@link IContract IContract}.
     */
    IContract build();
     ...
     * function use.
     *
     * @return the class {@link ContractResponse} containing all
     * relevant information wrapped by the class {@link ApiMsg ApiMsg}.
     */
    ApiMsg execute();

    /**
     * GetEncoded gets the input parameters after executed built transaction and
     * then set the return value to TxArgs's data field.
     * {@link TxArgs TxArgs}.
     *
     ...
     *
     * @return the interface {@link List List} of the class {@link ISolidityArg ISolidityArg}, need to cast to response
     * solidity type by the contract function.
     */
     ...
     *
     * @return the interface {@link List List} of the class {@link ISolidityArg ISolidityArg}, need to cast to response
     * solidity type by contract function.
     */
     ...
     *
     * @param e
     *      the class {@link String String} represent to the event name.
     *
     * @return the interface {@link IContract IContract}
     */
     ...
     *
     * @param e
     *      the interface {@link List List} of the class {@link String String} represent to the event name.
     *
     * @return the interface {@link IContract IContract}
     */
    ...
    /**
     * register the contract events for listening the events set by the method {@link IContract#newEvent(String)}, {@link IContract#newEvents(List)}
     * and {@link IContract#allEvents} to the Aion network and listening events until deregister it.
     *
     * @return the boolean value represent the status of the register wrapped by the class {@link ApiMsg}.
     */
    ApiMsg register();

    /**
     * register the contract events for listening the events set by the method {@link IContract#newEvent(String)}, {@link IContract#newEvents(List)}
     * and {@link IContract#allEvents} to the Aion network and listening events until the given block number as a string.
     *
     * @param s
     *      the class {@link String String} represent the block number.
     * @return the boolean value represent the status of the register wrapped by the class {@link ApiMsg}.
     */
    ...
    /**
     * register the contract events for listening the events set by the method {@link IContract#newEvent(String)}, {@link IContract#newEvents(List)}
     * and {@link IContract#allEvents} to the Aion network and listening events until the given block number as a class {@link String String}.
     *
     * @param ef
     *      the class {@link ContractEventFilter ContractEventFilter} represent the event listening condition such as the start block number
     *      , the end block number, or the specific address.
     * @return the boolean value represent the status of the register wrapped by the class {@link ApiMsg}.
     */
     ...
     *
     * @param e
     *      the interface {@link List List} of the class {@link String String} represent the event name.
     * @return the boolean value represent the status of the register wrapped by the class {@link ApiMsg}.
     */
     ...
     *
     * @return the interface {@link List List} of the class {@link ContractEvent ContractEvent} represent the happened events.
     */
     ...
     *
     * @return the interface {@link List List} of the class {@link String String} represent the event name.
     */
